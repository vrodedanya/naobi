...
void naobi::event_manager::updateContexts(std::list<workflow_context::sptr>&_contexts)
{
	while (!_events.empty())
	{
		LOG(event_manager.updateContexts, logger::LOW, "Event: ", _events.front());
		...
		{
			LOG(event_manager.updateContexts, logger::LOW, "Find workflow ", workflowIterator->second->name());
...
{
	LOG(event_manager.addWorkflow, logger::IMPORTANT, "Add workflow ", workflow->name(), " on target ", event);
