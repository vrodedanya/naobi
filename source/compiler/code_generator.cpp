	...
	_generatorRules(
		{
			...
			{[](const std::vector<std::string>& words) -> bool
			 ...
			 [this](
				 ...
				 std::vector<naobi::command>& commands)
			 {
				 assignmentLogic(words, commands);
			 }},
		})
{
}
...
naobi::code_generator::callFunction(const std::vector<std::string>& functionCallWords, std::vector<command>& commands)
{
	...
	if (functions.empty())
	{
		...
		NCRITICAL(code_generator, errors::INVALID_ARGUMENT,
				  "CRITICAL can't find function with arguments number '",
				  args.size(), "'");
	}
...
bool naobi::code_generator::generateFunction(const std::vector<std::string>& functionCallWords)
{
	...
	else
	{
		...
		else
		{
			...
			if (it == alreadySubstituted.end())
			{
				NLOG(code_generator, logger::IMPORTANT,
					 "WARNING failed to substitute type of template which is ",
					 templateFunction->getReturnType());
				return false;
			}
...
void naobi::code_generator::creatingVariableLogic(
...
void naobi::code_generator::ifElseLogic(const std::vector<std::string>& words, std::vector<naobi::command>& commands)
{
...
void naobi::code_generator::forLogic(const std::vector<std::string>& words, std::vector<naobi::command>& commands)
{
...
void naobi::code_generator::ariseLogic(const std::vector<std::string>& words, std::vector<naobi::command>& commands)
{
...
void naobi::code_generator::insertLogic(const std::vector<std::string>& words, std::vector<naobi::command>& commands)
{
...
void naobi::code_generator::catchLogic(const std::vector<std::string>& words, std::vector<naobi::command>& commands)
{
...
void naobi::code_generator::throwLogic(const std::vector<std::string>& words, std::vector<naobi::command>& commands)
{
...
void
naobi::code_generator::assignmentLogic(const std::vector<std::string>& words, std::vector<naobi::command>& commands)
{
	if (words.size() < 3)
	{
		NCRITICAL(code_generator, errors::WRONG_FORMAT,
